...
namespace NLinear
{
        ...
        where T : IEquatable<T>
    {
        ...
        public Vector3<T> ClosestPointTo(Line3<T> line, Func<double, T> fromDouble = null, Func<T, double> magnitude = null)
        {
            if (fromDouble == null)
            ...
            Vector3<T> posLpos = from - line.from;
        ...
        // in 3d; will also work in 2d if z components are 0
        {
            ...
            //Check with
            T v = a.DistanceTo(b);
            double valRet = Convert.ToDouble(v);
            //if(val > 0)
            ...
            if (Convert.ToDouble(val) != 0.0) // lines are not coplanar
                return false;
           // Point s = dot(cross(dc, db), cross(da, db)) / norm2(cross(da, db));
            ...
            Numeric<T> valN = val;
            valN /= temp2.Norm2();
            double vald = Convert.ToDouble(val);
